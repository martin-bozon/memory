                ...
                <form action="historique.php#class_perso" method="POST">
                    <select name="paire_joueur" id="">
                        ...
                        for ($i = 3; $i <= $nb_paire["nb_paire"] and $i <= 15; $i++) {
                            ...
                            <option value="<?= $i ?>"
                                <?php
                                if (isset($_POST["paire_joueur"]) && $i == $_POST["paire_joueur"]) {
                                    ?>
                                    selected
                                    <?php
                                }
                ...
                if (isset($_POST["valid_top_paire"], $_POST["paire_joueur"]) && !empty($top_paire_j)) {
                    ...
                    <table class="table table-dark table-striped">
                        ...
                        for ($i = 0; $i < $nb_score; $i++) {
                            ...
                            $general_paire = $bdd->query(
                                'SELECT count(score) as place FROM score WHERE score>? AND nb_paires=?',
                                [$top_paire_j[$i]["score"], $_POST["paire_joueur"]]
                            )->fetch(PDO::FETCH_ASSOC);
                            ?>
                            <tr>
